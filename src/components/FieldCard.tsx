import { MoreHorizontal, MapPin } from "lucide-react";
import { Field } from "@/data/fields";

function getPolygonPoints(coordinates: [number, number][][]): string {
  const coords = coordinates[0];
  if (!coords || coords.length < 3) return "10,30 20,8 35,25 25,35";
  const lngs = coords.map(c => c[0]);
  const lats = coords.map(c => c[1]);
  const minLng = Math.min(...lngs);
  const maxLng = Math.max(...lngs);
  const minLat = Math.min(...lats);
  const maxLat = Math.max(...lats);
  const rX = maxLng - minLng || 0.001;
  const rY = maxLat - minLat || 0.001;
  const unique = coords.length > 3 && coords[0][0] === coords[coords.length - 1][0] && coords[0][1] === coords[coords.length - 1][1]
    ? coords.slice(0, -1) : coords;
  return unique.map(c => {
    const x = 4 + ((c[0] - minLng) / rX) * 32;
    const y = 36 - ((c[1] - minLat) / rY) * 32;
    return `${x.toFixed(1)},${y.toFixed(1)}`;
  }).join(" ");
}

interface FieldCardProps {
  field: Field;
  onRemove: (id: string) => void;
  variant?: "select" | "list";
  isActive?: boolean;
  style?: React.CSSProperties;
}

const FieldCard = ({ field, onRemove, isActive = false, style }: FieldCardProps) => {
  const svgPoints = getPolygonPoints(field.coordinates);
  const areaDunam = Math.round(field.area * 4 * 10) / 10;

  return (
    <div
      className={`flex items-center gap-3 p-3 rounded-lg border transition-all duration-200 cursor-pointer group ${
        isActive ? "bg-primary/12 border-primary/60" : "bg-card/80 border-border hover:bg-accent/30"
      }`}
      style={style}
      dir="rtl"
    >
      <div className="w-12 h-12 rounded-md flex items-center justify-center flex-shrink-0 overflow-hidden bg-primary/10">
        <svg viewBox="0 0 40 40" className="w-8 h-8">
          <polygon points={svgPoints} fill={`${field.color}44`} stroke={field.color} strokeWidth="2" />
        </svg>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <span className="text-sm font-semibold text-foreground truncate">{field.name}</span>
          <span className="text-xs text-primary whitespace-nowrap">{areaDunam} دونم</span>
        </div>
        <div className="text-xs text-muted-foreground mt-0.5">{field.crop || "محصول غير محدد"}</div>
        <div className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
          <MapPin className="w-3 h-3 text-muted-foreground flex-shrink-0" />
          <span className="truncate">{field.location}</span>
        </div>
      </div>
      <button
        onClick={(e) => { e.stopPropagation(); onRemove(field.id); }}
        className="text-muted-foreground hover:text-foreground transition-all duration-200 flex-shrink-0 opacity-70 group-hover:opacity-100"
        aria-label="تعديل الأرض"
      >
        <MoreHorizontal className="w-4 h-4" />
      </button>
    </div>
  );
};

export default FieldCard;
